...
const Definitions = () => {
  ...
  const getRelatedConcepts = (term: string) => {
    ...
    const words = term
      ...
      .split(/\s+/)
      .filter((w) => w.length > 3);
    const related = allFilteredItems.filter(
      (item) =>
        item.term !== term &&
        words.some(
          (word) =>
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Your structured learning library extracted from notes.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-muted/30 rounded-xl border border-dashed">
            ...
            <p className="text-muted-foreground">
              Add definitions in your notes to see them here.
            </p>
          </div>
        ) : (
                              ...
                              <div className="mt-4 pt-4 border-t space-y-2">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {getRelatedConcepts(item.term).length > 0 ? (
                                    getRelatedConcepts(item.term).map(
                                      (rel: any) => (
                                        <Badge
                                          key={rel.id}
                                          variant="secondary"
                      ...
                      <Card className="bg-accent/5 border-dashed border-accent/20">
                        <CardHeader className="pb-2">
                          ...
                          <CardDescription className="text-[10px]">
                            Visualizing how these concepts build upon each other
                            (Heuristic analysis)
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
